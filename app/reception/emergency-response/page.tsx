...
export default function ReceptionEmergencyResponsePage() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Failed to load reception emergency response', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const acknowledgeAlert = async (requestId: number, alertId?: number | null) => {
    ...
    try {
      const res = await fetch('/api/reception/emergency-response', {
        method: 'POST',
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between rounded-xl border bg-card p-4">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => router.push('/reception/dashboard')}>
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <h1 className="text-xl font-bold text-primary">Reception Emergency Command</h1>
